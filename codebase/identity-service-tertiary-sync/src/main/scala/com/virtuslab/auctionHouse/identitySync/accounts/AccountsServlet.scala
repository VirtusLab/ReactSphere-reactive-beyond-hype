...
class AccountsServlet extends BaseServlet {
  ...
  post("/") {
    ...
    log.info(s"[${traceId.id}] Received account creation request for user '${accountRequest.username}' ...")

    val attempt = Try(accountService.createAccount(accountRequest))
      .map(_ => {
        log.info(s"[${traceId.id}] Account for '${accountRequest.username}' created.")
        Created()
      })
      .recover { case e: DuplicatedEntityException =>
        log.warn(s"[${traceId.id}] Account for '${accountRequest.username}' already exists.")
        BadRequest(ErrorResponse(e.getMessage))
      }

    if (attempt.isFailure) {
      log.error(s"[${traceId.id}] Error occured while creating account for user '${accountRequest.username}':", attempt.failed.get)
    }
