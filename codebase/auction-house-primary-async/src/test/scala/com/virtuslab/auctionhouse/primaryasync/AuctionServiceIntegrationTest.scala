...
import com.virtuslab.{CassandraQueriesMetrics, Logging, TraceId}
...
class AuctionServiceIntegrationTest extends WordSpec with Matchers with ScalaFutures with GivenWhenThen
  with OptionValues with CassandraIntegrationTest with AuctionServiceImpl with CassandraQueriesMetrics with Logging {
